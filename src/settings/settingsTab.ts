    ...
    display(): void {
        ...
        Sortable.create(cMenuToolbarCommandsContainer, {
            ...
            onSort: (command) => {
                if (command.from.className === command.to.className) {
                    ...
                    const [removed] = arrayResult.splice(command.oldIndex as number, 1)
                    arrayResult.splice(command.newIndex as number, 0, removed);
                    this.plugin.saveSettings();
                }
                setTimeout(() => {
                    dispatchEvent(new Event("cMenuToolbar-NewCommand"));
        ...
        this.plugin.settings.menuCommands.forEach((newCommand: Command, index: number) => {
            ...
            if ("SubmenuCommands" in newCommand) {
                    ...
                    onSort: (command) => {
                        ...
                        if (command.from.className === command.to.className) {
                            ...
                            if (subresult) {
                                ...
                                const [removed] = subresult.splice(command.oldIndex as number, 1);
                                subresult.splice(command.newIndex as number, 0, removed);
                                this.plugin.saveSettings();
                            }
                            ...
                            if (!command.target.parentElement) return
                ...
                if (!newCommand.SubmenuCommands) return
