...
缩放就多一组工具栏。现改为收集主窗口与所有弹出窗口的文档统一清理，
并在插入前移除目标容器内同样式的旧工具栏（幂等守卫）。
### resize 改为轻量重排
宽度变化不再销毁重建工具栏，只移动溢出按钮与主栏之间的分布
（200ms 防抖），消除闪烁；结构异常时自动回退完整重建。
### 移除未使用的依赖与安全加固
删除 react 系列等 6 个零引用依赖，运行时依赖漏洞审计归零；
移除误导入的 Node.js cluster 模块；动态 <style> 注入迁入 styles.css；
innerHTML 统一走集中式写入助手；接入 Obsidian 官方 eslint 规则。
